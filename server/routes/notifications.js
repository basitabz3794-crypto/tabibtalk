...
router.get('/me', requireLogin, async (req, res) => {
  ...
  const all = (await store.listNotifications()).slice(0, 30); // most recent first, capped
...
router.post('/mark-read', requireLogin, async (req, res) => {
  await store.updateUser(req.session.userId, { lastNotifSeenAt: new Date().toISOString() });
  res.json({ ok: true });
});
